...
- 이전 출력이 스크롤백에 남고, 각 입력 결과 화면이 transcript처럼 아래에 이어지는 terminal UX
- 서버는 여전히 현재 화면 스냅샷을 완전히 결정하고, 클라이언트는 이를 append 가능한 출력 블록으로 렌더링한다
- 입력 모드는 `line`과 `multiline` 두 가지
- `multiline` 편집 중 일반 본문 줄 입력은 세션 상태만 갱신하고, 응답은 `accepted`로 돌려 화면 재출력을 생략한다
...
- `POST /chol/sessions/{sessionId}/events`
  - 요청: `{ input }`
  - 응답: `{ kind: "screen", screen } | { kind: "accepted" }`
- `DELETE /chol/sessions/{sessionId}`
  - 응답: `{ ok, deleted }`

`kind: "screen"` 응답의 `screen`은 다음 필드를 가진다.
